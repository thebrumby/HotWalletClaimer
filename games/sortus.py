...
def fetch_and_process_logs(process_name):
    ...
    for line in relevant_lines:
        if "BALANCE:" in line and balance is None:
            balance = extract_detail(line, "BALANCE")
        if "STATUS:" in line and log_status is None:
            log_status = extract_detail(line, "STATUS")
        if "Need to wait until" in line and next_claim_at is None:
...
def display_processes(processes, status):
    ...
    for process_name in processes.splitlines():
        if process_name.strip():
            ...
            if next_claim_at:
                process_data.append((name, balance, next_claim_at, log_status))

    # Sort processes by next_claim_at (index 2 in tuple, which is datetime object)
    sorted_processes = sorted(process_data, key=lambda x: x[2])

    for name, balance, next_claim_at, log_status in sorted_processes:
        name = truncate_and_pad(name, name_width)
        balance = truncate_and_pad(balance, balance_width)
        next_claim_at = next_claim_at.strftime("%d %B - %H:%M") if next_claim_at else "None"
